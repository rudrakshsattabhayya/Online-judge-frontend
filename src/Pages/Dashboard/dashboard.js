...
import {
  removeNewLoginStatus,
  removeErrorStatus,
} from "../../redux/loginPageSlice";
import {
  problemTableThunk,
  filterTagsThunk,
  leaderBoardThunk,
  removeErrorStatusForDashboard,
...
const Dashboard = () => {
  ...
  const dashboardErrorMsg = useSelector(
    (status) => status.dashboardState.errorMsg
  );
  const problemPageStatus = useSelector(
    (status) => status.problemPageState.status
  );
  const problemPageErrorMsg = useSelector(
    (status) => status.problemPageState.errorMsg
  );
  const adminAuthStatus = useSelector((status) => status.adminAuthState.status);
  const adminAuthErrorMsg = useSelector(
    (status) => status.adminAuthState.errorMsg
  );
  const problemsState = useSelector(
    (status) => status.dashboardState.problemsTable
  );
  const filterValues = useSelector(
    (status) => status.dashboardState.filterValues
  );
  ...
  useEffect(() => {
    ...
    if (filterValues.difficulty > 0) {
      filteredProblemsState = filteredProblemsState.filter(
        (elem) => elem.difficulty === filterValues.difficulty
      );
    } else {
    ...
    filteredProblemsState.forEach((obj, index) => {
      ...
      if (obj.totalSubmissions !== 0) {
        const rate = parseInt(
          100 * (obj.acceptedSubmissions / obj.totalSubmissions)
        );
        successRate = `${rate}%`;
      }
      let difficulty = "Easy";
      ...
      {dashboardStatus !== STATUS.IDLE ? (
        ...
        <div id="dashboardContainer">
          <div id="dasboardHeading">
            {screenWidth <= 900 && (
              <MenuIcon id="dashboardMenuIcon" onClick={handleMenuIcon} />
            )}
            <p>Dashboard</p>
          </div>
          <div id="dashboardContent">
            <Table
              className="dashboardContentItem dashboardContentTable"
              tableHeading="Problems"
              headingRowInfo={headingRowInfo}
              trInfo={problems}
            />
